...
print(f'произведение нужных символов из файла = {result_d}')
# 5 реализовать перемешивание списка
f = open('file_1.txt', 'r')
list_for_shake = list(int(line[:]) for line in f)
print(list_for_shake)
random.shuffle(list_for_shake)
print(list_for_shake)
for i in range(len(list_for_shake)-1):
    list_for_shake[i], list_for_shake[random.randint(0, len(list_for_shake)-1)] = list_for_shake[random.randint(0, len(list_for_shake)-1)], list_for_shake[i]
print(list_for_shake)